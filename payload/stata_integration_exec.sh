...
## query whether to install mimetypes, file associations, and icons system wide or for the current user
if [ -z "${ARGMODE}" ] ; then
		prompt_msg "$(printf "(4) Please specify if we should install file type associations and icons for the current user ('user'), or system-wide ('system'; this will need to ask for a 'sudo' or root password during setup) [%s]:" "${DEFAULTMODE}")"
		read -r QUERIEDMODE
		case ${QUERIEDMODE} in
			"" )
				MODE="${DEFAULTMODE}"
	...
	case ${ARGMODE} in
		...
		;;
		* )
			error_msg "$(printf 'Invalid argument for option \e[1m--mode\e[0m: \e[1m%s\e[0m' "${ARGMODE}")"
			exit ${EXIT_ERROR}
		;;
	esac
	status_msg "$(printf "You already answered (4) via command line: \e[1m--mode %s\e[0m" "${ARGMODE}")"
fi
## query users to create file type associations for
if [ -z "${ARGUSERS}" ] ; then
	prompt_msg "$(printf "(5) Please specify a space-separated (!) list of all users you want to create file type associations for [%s]:" "${USER}")"
...
if [ "${LIBPNGFIX}" = "true" ]; then
	status_msg "$(printf "Starting to apply workaround for PNG icons not showing up correctly in Stata GUI...\n\tThis requires write privileges to Stata's installation directory!")"
	if [ -x "${INSTALLPATH}/libpngworkaround" ]; then
		if ! rm -rf "${INSTALLPATH}/libpngworkaround"
		then
			elevate_cmd rm -rf "${INSTALLPATH}/libpngworkaround"
		fi
	fi
	if ! mkdir "${INSTALLPATH}/libpngworkaround"
	then
		elevate_cmd mkdir "${INSTALLPATH}/libpngworkaround"
	fi
	BUILDDIR="${PAYLOADPATH}/build"
	CWD=$(pwd)
	if ! mkdir "${BUILDDIR}"
	then
		elevate_cmd mkdir "${BUILDDIR}"
	fi
	cd "${BUILDDIR}" || exit
...
for FILE in "${ICONPATH}"/png/*.png ; do
	...
	if [ "${TARGET}" = 'statalogo' ] ; then
		...
		exec_depending_on_mode  xdg-icon-resource install --noupdate --context apps --mode "${MODE}" --size "${HPIXEL}" "${FILE}" "application-x-stata-stata${VERSION}logo"
		if [ "${CURRENT_ICON_THEME}" != "hicolor" ]; then
			status_msg "$(printf '\t\t...to the current theme: \e[1m%s\e[0m' "${CURRENT_ICON_THEME}")"
			exec_depending_on_mode  xdg-icon-resource install --theme "${CURRENT_ICON_THEME}" --noupdate --context apps --mode "${MODE}" --size "${HPIXEL}" "${FILE}" "application-x-stata-stata${VERSION}logo"
		fi
	else
		if [ "${TARGETLIST#*"${TARGET}"}" = "${TARGETLIST}" ] ; then
			...
			exec_depending_on_mode  xdg-mime install --mode "${MODE}" "${PAYLOADPATH}/mimetypes/stata-statamimetype_${TARGET}.xml"
		...
		status_msg "$(printf '\t\t...to the fallback theme: \e[1mhicolor\e[0m')"
		exec_depending_on_mode  xdg-icon-resource install --noupdate --context mimetypes --mode "${MODE}" --size "${HPIXEL}" "${FILE}" "application-x-stata-${TARGET}"
		if [ "${CURRENT_ICON_THEME}" != "hicolor" ]; then
			status_msg "$(printf '\t\t...to the current theme: \e[1m%s\e[0m' "${CURRENT_ICON_THEME}")"
			exec_depending_on_mode  xdg-icon-resource install --theme "${CURRENT_ICON_THEME}" --noupdate --context mimetypes --mode "${MODE}" --size "${HPIXEL}" "${FILE}" "application-x-stata-${TARGET}"
		fi
	fi
done
...
exec_depending_on_mode  xdg-desktop-menu install --noupdate --mode "${MODE}" "${PAYLOADPATH}/stata-stata${VERSION}_console.desktop"
exec_depending_on_mode  xdg-desktop-menu forceupdate
...
for TARGET in ${TARGETLIST} ; do
	...
	for TARGETUSER in ${TARGETUSERS} ; do
		...
		if [ "${TARGETUSER}" = "${USER}" ] ; then
			...
			elevate_cmd -u "${TARGETUSER}" -H xdg-mime default stata-stata"${VERSION}"_windowed.desktop "${MIMETYPE}"
		fi
	done
done
status_msg "$(printf '...finished setting default application for mimetypes')"
status_msg "$(printf 'refreshing icon database...')"
exec_depending_on_mode xdg-icon-resource forceupdate --mode "${MODE}"
...
exec_depending_on_mode update-mime-database "${MIMEDIR}"
